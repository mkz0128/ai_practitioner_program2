...
test('R3-1：交錯操作不污染脈絡', async ({ page }) => {
  ...
  const summary = await sendUi(page, 'ORD-101，信義示範配送點，臺北市，行政區信義，25.033，121.565，Z3，1 件')
...
test('R3-3：補欄位中插入查詢後仍可完成草稿', async ({ page }) => {
  ...
  const completed = await sendUi(page, 'ORD-101，信義示範配送點，臺北市，25.033，121.565，Z3，1 件')
