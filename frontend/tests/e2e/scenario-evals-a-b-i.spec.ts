...
async function upload(page: Page, workbook: string, expected: string) {
  ...
  await expect(page.locator('.topbar-stats')).toContainText(expected, { timeout: 180_000 })
...
test('A 區塊：插單對話、缺欄契約與兩則訊息摘要', async ({ page }) => {
  ...
  // 缺欄清單改成講人話的分組：地點名稱／城市／行政區併成「配送地點」，
  // 經緯度併成「座標」。已經講過的重量不會再問，優先程度也不由調度員填。
  for (const label of ['訂單編號', '配送地點', '座標']) {
    expect(missing.message || '', `系統實際回覆：${missing.message}`).toContain(label)
  }
  expect(missing.message || '').not.toContain('重量')
  ...
  // 這份資料裡「信義」同時屬於 Z2（東區）與 Z3（中區），光講行政區推不出
  // 責任區，所以還是要問。要守住的是：講了行政區就不會再問行政區。
  expect(missingByOrder[0]?.missing_fields || []).not.toContain('district')
...
test('A-09：有完整座標時不追問地點名稱', async ({ page }) => {
  ...
  await upload(page, relaxedWorkbook, '50/50 已安排')
  const completion = await send(page, '新增急單 ORD-A09，臺北市，行政區信義，配送區域 Z3，座標 25.033 / 121.565，1 件 15 公斤，早上時段')
  expect(completion.message || '').toContain('我記下來了，確認一下')
...
test('I 區塊：tight 的排不進去與三公里以上取捨', async ({ page }) => {
  ...
  const heavy = await send(page, '新增急單 ORD-I03，配送區域 Z2，城市臺北市，行政區內湖，地點名稱內湖超重示範站，緯度 25.083，經度 121.590，包裹件數 1，每件重量 200 公斤，早上配送')
