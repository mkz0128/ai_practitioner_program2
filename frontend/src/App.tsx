...
export default function App() {
  ...
  const handlePreview = async () => {
    ...
    try { setPreview(await previewUrgent(plan.plan_id, plan.version)) }
    catch (requestError) { setError(errorText(requestError)) }
    finally { setBusy(false) }
  }
  ...
  return <div className="app-shell">
    <StatusBar plan={plan} providers={providers} />
    <main className="workspace">
      <AgentPanel plan={plan} onChat={handleChat} onImport={handleImport} busy={busy} />
      <MapPanel data={mapData} activeVehicle={activeVehicle} onSelectVehicle={setActiveVehicle} />
      <VehiclePanel plan={plan} activeVehicle={activeVehicle} onSelectVehicle={setActiveVehicle} />
    </main>
    <DetailsPanel plan={plan} preview={preview} onPreview={handlePreview} onConfirm={handleConfirm} busy={busy} />
