...
describe('配送調度控制塔 v2 前端', () => {
  ...
  it('開啟後維持空白控制塔，等待使用者提供資料', () => { render(<App />); expect(screen.getByText('先放入今天的訂單')).toBeInTheDocument(); expect(screen.getByRole('textbox', { name: '輸入訊息' })).toBeInTheDocument(); expect(screen.queryByText('下載範例格式')).not.toBeInTheDocument(); expect(screen.queryByText('—')).not.toBeInTheDocument() })
  it('使用者選取有效 Excel 後呈現方案、車輛與地圖', async () => { render(<App />); const file = new File(['xlsx'], 'demo.xlsx', { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }); fireEvent.change(screen.getByLabelText('上傳 Excel'), { target: { files: [file] } }); fireEvent.click(screen.getByRole('button', { name: '送出' })); await waitFor(() => expect(screen.getByText('車輛概況')).toBeInTheDocument()); expect(screen.getByText('1/1 已安排')).toBeInTheDocument(); expect(api.importWorkbook).toHaveBeenCalledTimes(1); expect(api.createPlan).toHaveBeenCalledWith('DATA-TEST', 'BALANCED', expect.any(AbortSignal)) })
})
