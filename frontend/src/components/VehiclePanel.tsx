...
export function VehiclePanel({ plan, activeVehicle, onSelectVehicle }: VehiclePanelProps) {
  return (
    <section className="panel" aria-label="車輛與例外案件">
      <div className="panel-heading"><div><h2>車輛與例外案件</h2><p>載重、服務區域與合法性</p></div>{plan && <span className={`status-chip ${plan.validation.valid ? 'live' : 'blocked'}`}>{plan.validation.valid ? 'Validator 通過' : '需人工複核'}</span>}</div>
      <div className="panel-body">
        ...
        <div className="vehicle-list">
          {plan?.vehicles.map((vehicle) => <button className={`vehicle-card ${activeVehicle === vehicle.vehicle_id ? 'active' : ''}`} key={vehicle.vehicle_id} onClick={() => onSelectVehicle(vehicle.vehicle_id)}>
            <div className="vehicle-title"><span>{vehicle.vehicle_id} · {vehicle.vehicle_name}</span><span>{(vehicle.load_utilization * 100).toFixed(1)}%</span></div>
            <div className="vehicle-meta">服務區域：{vehicle.service_zone_codes.join('、') || '—'}</div>
            <div className="progress"><span style={{ width: `${Math.min(100, vehicle.load_utilization * 100)}%` }} /></div>
            <div className="vehicle-stats"><span>{vehicle.planned_load_kg.toFixed(1)} / {vehicle.max_load_kg.toFixed(1)} kg</span><span>{vehicle.order_count} 張訂單 · {vehicle.total_distance_m.toLocaleString()} m</span></div>
          </button>)}
        </div>
