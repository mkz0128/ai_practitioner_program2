...
const zoneNames: Record<string, string> = { Z1: '北區', Z2: '東區', Z3: '中區', Z4: '西區', Z5: '南區' }

// 每一欄的欄頭就是那台車的儀表：載重、站數、里程、時間、責任區。
// 這些數字原本在另一張「車輛概況」卡，同一批車在畫面上講了三遍。
function ColumnHead({ vehicle }: { vehicle: Plan['vehicles'][number] }) {
  const percent = Math.round(vehicle.load_utilization * 100)
  const tight = vehicle.load_utilization >= 0.9
  return (
    <div className="order-board-heading">
      <div className="obh-top">
        <strong>{vehicleLabel(vehicle.vehicle_id)}</strong>
        <span className={cn('obh-pct', tight && 'obh-pct-tight')}>{percent}%</span>
      </div>
      <div className="obh-bar"><i style={{ width: `${Math.min(100, percent)}%` }} className={tight ? 'tight' : undefined} /></div>
      <div className="obh-meta">
        <span>{vehicle.order_count} 站</span>
        <span>{formatWeight(vehicle.planned_load_kg)} / {formatWeight(vehicle.max_load_kg)}</span>
        <span>{formatDistance(vehicle.total_distance_m)}</span>
        <span>{formatDuration(vehicle.total_duration_s)}</span>
      </div>
      <div className="obh-zones">{vehicle.service_zone_codes.map((code) => zoneNames[code] || code).join('、') || '未指定'}</div>
    </div>
  )
}
