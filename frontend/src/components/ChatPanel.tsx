...
export function ChatPanel({ onChat, onInspectFile, onImportFile, onConfirmOption, onConfirmRule, onConfirmDeviation, onManualAdjust, busy, onStop, plan, activity, messages: controlledMessages, setMessages: controlledSetMessages }: ChatPanelProps) {
  /* 對話紀錄由上層持有。排班一完成，App 就從空白殼換成正式版面，那是兩棵不同
     ...
     話進來。沒接上層時退回自己管，測試單獨掛這個元件仍然可用。 */
  const [fallbackMessages, setFallbackMessages] = useState<ChatMessage[]>([])
  const messages = controlledMessages ?? fallbackMessages
  const setMessages = controlledSetMessages ?? setFallbackMessages
      ...
      if (selectedFile) {
        try {
          ...
          if (mapping?.requires_confirmation) {
            ...
            /* 這句是進度，要在等待之前就寫進去。原本寫在 await 之後，
               而匯入完成時 App 會接著補一句排班結果——那一句才是最後一則，
               這裡再去改「最後一則」就把結果蓋掉了，畫面只剩這句進度。 */
