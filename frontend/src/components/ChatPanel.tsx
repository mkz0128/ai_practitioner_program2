...
import { formatEta, formatNumber, vehicleLabel, cn } from '../lib/utils'
import { fieldLabel, fieldLabels, formatValidationReport } from '../lib/fieldLabels'
...
function optionAssignment(option: UrgentPlanOption): string {
  ...
  return option.inserted_orders.map((item) => item.status === 'ASSIGNED' ? `${item.order_id} → ${vehicleLabel(item.vehicle_id)} 第 ${item.sequence} 站${item.responsibility ? `（${item.responsibility}）` : ''}` : `${item.order_id} → 需人工處理`).join('；')
}
...
export function ChatPanel({ onChat, onInspectFile, onImportFile, onConfirmOption, onConfirmRule, onConfirmDeviation, onManualAdjust, busy, onStop, plan, activity, initialMessages, onMessagesChange }: ChatPanelProps) {
  /* 對話紀錄存在上層。排班一完成，App 就從空白殼換成正式版面，那是兩棵不同的
     子樹，React 會把這個面板拆掉再裝一個新的——對話內容就跟著沒了。調度員剛
     丟完檔案，抬頭看到的是一個空的對話框。所以這裡只是接手上層那份紀錄。 */
