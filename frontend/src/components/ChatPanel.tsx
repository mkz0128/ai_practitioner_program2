...
import type { ChatResponse, ColumnMappingResponse, DispatchRuleEvidence, DispatchRuleOption, UrgentPlanOption } from '../types'
...
interface Message { role: 'user' | 'assistant'; text: string; attachment?: string; response?: ChatResponse; options?: UrgentPlanOption[]; ruleOptions?: DispatchRuleOption[]; ruleEvidence?: DispatchRuleEvidence }
export interface PendingMapping { file: File; response: ColumnMappingResponse }
interface ChatPanelProps { onChat: (message: string, action?: ChatAction) => Promise<ChatSubmitResult>; onInspectFile: (file: File) => Promise<ColumnMappingResponse | null>; onImportFile: (file: File, mapping?: Record<string, Record<string, string>>, mappingName?: string) => Promise<void>; onConfirmOption?: (option: UrgentPlanOption) => Promise<void>; onConfirmRule?: (option: DispatchRuleOption) => Promise<void>; busy: boolean; onStop: () => void; plan: boolean }
...
function responseCards(response: ChatResponse): { urgent: UrgentPlanOption[]; rules: DispatchRuleOption[]; ruleEvidence?: DispatchRuleEvidence } {
  ...
  return { urgent, rules, ruleEvidence }
}
...
function optionAssignment(option: UrgentPlanOption): string {
  ...
  return option.inserted_orders.map((item) => item.status === 'ASSIGNED' ? `${item.order_id} → ${item.vehicle_id} 第 ${item.sequence} 站` : `${item.order_id} → 需人工處理`).join('；')
}
...
function OptionCards({ options, onConfirmOption }: { options: UrgentPlanOption[]; onConfirmOption?: (option: UrgentPlanOption) => Promise<void> }) {
  ...
  return <div className="mt-4 space-y-3" role="group" aria-label="臨時插單方案">{options.map((option) => {
    ...
    if (!option.selectable) return <div className="urgent-card urgent-card-unavailable" key={option.option_id}>{content}</div>
    return <div key={option.option_id}><button type="button" aria-pressed={active} className={cn('urgent-card w-full text-left', active && 'urgent-card-selected')} onClick={() => setSelected(option.option_id)}>{content}</button>{active && <div className="flex items-center gap-2 rounded-b-lg bg-blue-50 px-3 py-2"><Button type="button" variant="secondary" disabled={confirmed === option.option_id} onClick={async () => { if (!onConfirmOption) return; await onConfirmOption(option); setConfirmed(option.option_id) }}>確認套用</Button><Button type="button" variant="ghost" onClick={() => setCancelled(true)}>取消</Button><span className="text-xs font-semibold text-blue-800">{confirmed === option.option_id ? '已建立新版本。' : '確認後才會建立新版本。'}</span></div>}</div>
...
<div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-0.5"><strong className="whitespace-nowrap text-slate-900">{String(option.label)}</strong><span className="whitespace-nowrap tabular-nums text-slate-500">目前 {String(option.current_value)} {String(option.unit)}</span></div>{Boolean(option.current_detail) && <span className="mt-1 block leading-5 text-slate-500">{String(option.current_detail)}</span>}</div>)}</div>}</div>
  ...
  return <div className="mt-3 space-y-3" role="group" aria-label="司機規則試算方案"><div className={cn('rounded-lg border px-3 py-3', evidence?.status === 'CONFLICT' ? 'border-amber-200 bg-amber-50' : 'border-blue-100 bg-blue-50')}><div className="flex items-start justify-between gap-3"><div><strong className="text-sm text-slate-900">{evidence?.rule?.summary || options[0]?.title || '規則試算'}</strong></div><Badge tone={evidence?.status === 'CONFLICT' ? 'warning' : 'success'}>{evidence?.status === 'CONFLICT' ? '有衝突' : '可試算'}</Badge></div>{evidence?.rule?.source_utterance && <p className="mt-2 text-xs text-slate-600">原句：{evidence.rule.source_utterance}</p>}{evidence?.trial && <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 border-t border-blue-100 pt-3 text-xs"><Metric label="影響" value={`${evidence.trial.affected_order_count} 張`} /><Metric label="未安排" value={`${evidence.trial.unassigned_orders.length} 張`} /><Metric label="距離" value={delta(evidence.trial.distance_delta_m / 1000, 'km')} /><Metric label="時間" value={delta(evidence.trial.duration_delta_s / 60, '分鐘')} /></div>}{conflicts.length > 0 && <div className="mt-3 rounded-lg border border-amber-200 bg-white px-3 py-2 text-xs leading-5 text-amber-900"><p className="font-semibold">衝突原因</p>{conflicts.map((conflict) => <p key={conflict.rule_id}>{conflict.rule_id}：{conflict.order_ids.join('、')}；{conflict.reason}</p>)}<div className="mt-2 flex flex-wrap gap-2">{resolutions.map((item) => <Button key={item.action} type="button" variant="outline" onClick={() => undefined}>{String(item.label)}</Button>)}</div></div>}{options.length > 0 && <div className="mt-3 flex items-center gap-2"><Button type="button" variant="secondary" disabled={confirmed} onClick={async () => { const option = options[0]; if (!onConfirmRule || !option) return; await onConfirmRule(option); setConfirmed(true) }}>套用</Button><span className="text-xs font-semibold text-blue-800">{confirmed ? '已套用；重新排班後生效。' : '試算通過後才會建立規則。'}</span></div>}</div></div>
}
...
export function ChatPanel({ onChat, onInspectFile, onImportFile, onConfirmOption, onConfirmRule, busy, onStop, plan }: ChatPanelProps) {
  ...
  const [thinkingPhase, setThinkingPhase] = useState('理解你的需求…')
  ...
  useEffect(() => () => { if (slowTimer.current !== null) window.clearTimeout(slowTimer.current) }, [])
  ...
  async function submit(message = text, selectedFile = file, action?: ChatAction) {
    ...
    setSlow(false); setThinkingPhase('理解你的需求…')
    ...
    try {
      ...
      setThinkingPhase('呼叫工具計算…')
      const result = await onChat(value, action)
      setThinkingPhase('整理結果…')
      const cards = result.response ? responseCards(result.response) : { urgent: [], rules: [], ruleEvidence: undefined }
      const assistantText = result.response ? evidenceMessage(result.response) : result.error?.trim() || '這則訊息未完成，請再試一次。'
      setMessages((items) => items.map((item, index) => index === items.length - 1 ? { ...item, text: assistantText, response: result.response || undefined, options: cards.urgent, ruleOptions: cards.rules, ruleEvidence: cards.ruleEvidence } : item))
    } finally {
      if (slowTimer.current !== null) window.clearTimeout(slowTimer.current)
      setSending(false)
    }
  ...
  return <Card className="flex min-h-[620px] flex-col" aria-label="AI 調度助理"><CardHeader><SectionTitle title="調度對話" detail={plan ? '所有方案先預覽，再由調度員確認。' : '上傳資料後，從這裡查看排班結果。'} /><Badge tone="success">可用</Badge></CardHeader><CardContent className="flex min-h-0 flex-1 flex-col gap-4"><div ref={log} className="chat-log" aria-live="polite">{messages.map((message, index) => <div key={`${message.role}-${index}`} className={cn('mb-4 max-w-[94%]', message.role === 'user' ? 'ml-auto' : 'mr-auto')}><div className={cn('rounded-2xl px-4 py-3 text-sm leading-6', message.role === 'user' ? 'rounded-br-md bg-slate-900 text-white' : 'rounded-bl-md border border-slate-200 bg-white text-slate-700')}><div className={cn('mb-1 text-[11px] font-bold', message.role === 'user' ? 'text-slate-300' : 'text-slate-400')}>{message.role === 'user' ? '你' : '調度助理'}</div>{message.attachment && <div className="mb-2 rounded-lg bg-white/10 px-3 py-2 text-xs">附件：{message.attachment}</div>}<div>{message.text || (isBusy && index === messages.length - 1 ? <><span>{thinkingPhase}</span><span className="chat-thinking-dots" aria-hidden="true">...</span>{slow && <span className="ml-2 text-xs text-amber-700">這題比較久，仍在計算</span>}</> : '')}</div>{message.response && isUrgentReviewReady(message.response) && <div className="mt-3"><Button type="button" variant="secondary" disabled={isBusy} onClick={() => void submit('產生插單預覽', undefined, 'PREVIEW_URGENT')}>產生插單預覽</Button></div>}{message.options && message.options.length > 0 && <OptionCards options={message.options} onConfirmOption={onConfirmOption} />}{(message.ruleOptions && message.ruleOptions.length > 0 || message.ruleEvidence) && <RuleCards options={message.ruleOptions || []} evidence={message.ruleEvidence} onConfirmRule={onConfirmRule} />}</div></div>)}</div>{pendingMapping && <MappingReview pending={pendingMapping} value={mappingDraft} onChange={updateMapping} name={mappingName} onNameChange={setMappingName} onConfirm={() => void confirmMapping()} onCancel={() => { setPendingMapping(null); setMappingDraft({}) }} busy={isBusy} />}{/* 次要動作集中在同一排，把整個寬度讓給輸入框——Demo 要打的是長句子 */}
