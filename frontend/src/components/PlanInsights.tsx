    ...
    <section className="panel plan-insights" aria-label="方案分析與版本">
      ...
      <div className="panel-body insight-body">
        <div className="insight-block">
          ...
          {delayPreview && <div className="insight-summary"><span>方案檢查：{delayPreview.validator.valid ? '通過' : '需人工複核'}</span><span>受影響：{String((delayPreview.simulation as Record<string, unknown>).affected_order_count ?? 0)} 張訂單</span><span>模擬延遲：{String((delayPreview.simulation as Record<string, unknown>).delay_minutes ?? '—')} 分鐘</span></div>}
        ...
        <div className="insight-block">
          <strong>方案比較</strong>
          {!comparison && <span className="hint">比較最快、均衡載重與穩定時段三種方案的實際取捨。</span>}
        ...
        <div className="insight-block">
          <strong>方案版本</strong>
          {!versions && <span className="hint">查看目前版本、歷史差異與復原選項。</span>}
          {versions && <div className="version-list">{versions.versions.map((version) => <div className="version-row" key={version.version}><span><b>V{version.version}</b><small>{version.state === 'CONFIRMED' ? '已確認' : '草稿／待確認'} · {version.validator_valid ? '方案檢查通過' : '需複核'}</small></span>{version.version < versions.current_version && <button type="button" className="filter-pill" onClick={() => void onRestore(version.version)} disabled={busy}>復原為此版本</button>}</div>)}</div>}
        </div>
      </div>
    </section>
  )
