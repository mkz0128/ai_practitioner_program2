  ...
  return (
    <section className="panel bottom-panel" aria-label="配送明細">
      ...
      <div className="panel-body">
        ...
        {plan && <div className="approval-bar"><span className="hint">{plan.state === 'CONFIRMED' ? '此方案已由調度員確認。' : '方案尚待人工確認；未確認前不會執行 Dispatch。'}</span><span>{preview && <><button className="control-button" onClick={() => void onConfirm()} disabled={busy || plan.state === 'DISPATCHED'}>人工確認預覽</button>{onCancelPreview && <button className="control-button ghost" onClick={() => void onCancelPreview()} disabled={busy}>取消預覽</button>}</>}</span></div>}
      </div>
    </section>
  )
}
