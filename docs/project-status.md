...
- Latest quality gates: `ruff check .` `PASS`; `mypy src` `PASS` (24 files); secret scan `PASS`; no Actions/deploy workflow; latest commit `ff72319` (status-only follow-up) pushed to `origin/main`; working tree clean before this status-only update.
